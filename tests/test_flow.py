...
class FakeChannel:
    ...
    async def send(self, content, reference=None, allowed_mentions=None):
        self.sent.append(SimpleNamespace(content=content, reference=reference, allowed=allowed_mentions))
...
def make_bot(wcl):
    config = Config("token", frozenset({CHANNEL}), None, None, None, ":memory:", 5, 20, 8, RecapSettings())
    bot = RecapBot(config)
    bot.wcl = wcl
    channel = FakeChannel()
...
def test_finished_log_is_posted_right_away_with_pings():
    ...
    msg = channel.sent[0]
    assert "<@111> 94.5" in msg.content
    assert msg.reference.message_id == 500  # replies to the link
    assert msg.allowed.users is True and msg.allowed.everyone is False and msg.allowed.roles is False
